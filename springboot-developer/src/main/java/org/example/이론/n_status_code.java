...
public class n_status_code {
    ...
    // - 403 Forbidden: 권한이 없어 접근 할 수 없음
    //      EX) 클라이언트가 요청한 리소스에 대한 접근 권한이 존재하지 않음
    //      , 서버는 요청을 이해했으나 권한이 부족하여 처리를 거부

    // - 404 Not Found: 요청한 리소스를 찾을 수 없음
    //      EX) 서버가 요청된 URI를 확인 했으나 , 해당하는 리소스가 존재하지 X

    // - 5xx (서버오류)
    // - 500 Interner Server Error: 서버가 요청을 처리하는 중에 오류가 발생
    // - 503 Service Unavailable: 서버가 일시적으로 과부화 또는 유지보수 중
}
