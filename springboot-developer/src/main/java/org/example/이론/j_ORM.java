...
public class j_ORM {
    ...
    // 1. ORM(Obkect-Relational Mapping)
    // : 객체와 관계형 데이터베이스 간의 데이터를 매핑해주는 기술
    // - 데이터베이스의 테이블과 애플리케이션 객체간의 구조적 불일치를 해결하는 솔루션

    // cf) RDBMS: user_password (lower_snake_case)
    // cf) JAVA: userPassword (lowerCamelcase)

    // 1) ORM의 특징과 장점
    // - 객체와 테이블이 매핑: 1:1로 연결하여 데이터를 매핑
    // - 객체지향적인 데이터 조작: SQL 대신 자바 객체 메서드로 CRUD 작업 수행
    // >> 반복적인 SQL 작성 없이 데이터 조작 가능
    // >> DBMS에 독립적
    // >> 학습 곡선 높음, 복잡한 쿼리 작성 어려움

    // 2) ORM의 동작 원리
    // : 각 테이블은 클래스에, 테이블의 각 행(row)는 객체에 매핑
    // - Entity: 데이터베이스 테이블과 매핑되는 클래스
    // - Field: 클래스의 필드는 테이블의 열(column)에 해당

    //2, JPA(Java Persistence API)
    // : ORM 의 종류중 하나
    // > java애플리케이션에서 관계형 데이터 베이스를 쉽게 다룰 수 있는 ORM 표준
...
// );
 ...
 //          }
    ...
    //  : 기본 키 자동 생성 전략 설정
    // +) 다양한 옵션이 있음
    //  - strategy옵션: GenerationType.AUTO.IDENTITY(auto_increment)

    // @Column
    // : 필드를 특정 테이블 열과 매핑
    // : 생략 시 기본으로 필드이름이 열 이름으로 사용
    // +) 다양한 옵션이 있음
    //   - name 옵션 : 열 이름 지정
    //   - nullable 옵션 : 열이 null값을 허용할 지 여부를 설정 (기본값 true)
    //   - length 옵션 : Sting 타입의 열 길이를 지정 (기본값 255)
    ...
    // 애플리케이션 설정: 연결한 DB와 사용자 정보, 비밀번호 등을 설정
}
