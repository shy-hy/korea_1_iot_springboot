...
public class WebSecurityConfig {
    ...
    @Autowired
    // 요청이 들어 올 때 JWT 토큰을 검증하는 필터
    // : UsernamePasswordAuthenticationFilter 이전에 동작 , JWT 토큰이 유효한지 검사하여 사죵자를 인증
    ...
    *
    * cf) CSRF(Cross-Site Request Forgery)(나쁜거) 공격: 사용자 대신 웹 애플리케이션에서 악의적인 행동을 하는 공격
    *   CORS(Cross-Origin Resource Sharing)(좋은거) 정책 : 서로 다른 서버간의 리소스 상호작용을 위한 정책
    *
    ...
    *   , BCryptPasswordEncoder를 사용하여 비밀번호를 암호화 하여 검증
    * */
    ...
    @Bean
    // BCryptPasswordEncoder: 비밀 번호 암호화에 사용되는 클래스
    // +) 복호화 : 암호를 복구하다
